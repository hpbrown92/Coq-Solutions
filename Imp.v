...
Proof.
  ...
  - simpl. rewrite optimize_0plus_sound. 
    destruct a;simpl;rewrite optimize_0plus_sound;reflexivity.
  - simpl. rewrite optimize_0plus_sound.
    destruct a0;rewrite optimize_0plus_sound;reflexivity.
...
Proof.
  ...
  -  apply E_WhileLoop with (t_update (t_update (t_update 
    ...
    + apply E_Seq with (t_update (t_update (t_update empty_state X 2) Y 0) Y 2).
      * apply E_Ass. reflexivity.
      * apply E_Ass. reflexivity.
    + apply E_WhileLoop with (t_update (t_update (t_update (t_update (t_update 
                             (t_update empty_state X 2) Y 0) Y 2) X 1) Y 3) X 0).
      * reflexivity.
...
| WR_Skip: no_whilesR SKIP
| WR_Ass: forall x y, no_whilesR (x::=y)
| WR_Seq: forall x y, no_whilesR x -> no_whilesR y -> no_whilesR(x;;y)
| WR_If: forall x y b, no_whilesR x-> no_whilesR y -> no_whilesR(IFB b THEN x ELSE y FI)
.
...
Proof.
  ...
  - apply WR_Skip.
  - apply WR_Ass.
  - simpl in H. apply andb_true_iff in H. destruct H. apply WR_Seq.
    + apply IHc1. apply H.
    + apply IHc2. apply H0.
  - apply andb_true_iff in H. destruct H. apply WR_If.
    + apply IHc1. apply H.
    + apply IHc2. apply H0.
...
(** Imp programs that don't involve while loops always terminate.
    State and prove a theorem [no _whiles_terminating] that says this. *)
