...
(** Write an informal proof of [double_neg]:
  ...
  _Theorem_: [P] implies [~~P], for any proposition [P].
   ...
   _Proof_: By propositional logic

   _ First, suppose we have proposition P which we know is provable,
            which follows from P -> P.

   _ Next, if we had a propositional statement P -> ~P,
          here P implies false by definition of not which gives a contradiction.

    _Finally, since we can't have this contradiction we can produce the proportional statement
              P -> ~~P, which produces our original statement P-> P or P-> True.
...
(** Write an informal proof (in English) of the proposition [forall P
  ...
  _Theorem_: The negation of P and ~P is false
  
  _Proof_: By propositional logic.

  _First, suppose we have the propositions P and ~P, 
          we know this is false.
  
  _Next, next if we negated these propositions P and ~P,
        we would find ourselves with ~P and ~~P, which would 
        give us both false and true.

  _Finally, since we have both false and true again,
            we known the propositional statement is false. 
*)
...
Proof.
  intros P.
  - split. intros H. apply H. intros H2. apply H2.
Qed.
...
(** **** Exercise: 1 star (dist_not_exis-ts)  *)
...
(** **** Exercise: 2 stars (In_map_iff)  Hint: use split*)
Lemma In_map_iff :
 ...
 - split. 
      * intros. simpl in H. destruct H. 
        ...
        { simpl. apply IHl' in H. destruct H.
            * exists x. split.
              - destruct H. apply H. 
              - right. destruct H. apply H0. }
       * intros. destruct H. destruct H. simpl in H0. destruct H0. 
              { simpl. left. rewrite -> H0. apply H. }
              { simpl. right. rewrite -> IHl'. exists x. split.
                * apply H.
                * apply H0. }
Qed.
(** [] *)
...
Proof.
  split; intros. 
  induction l.
  - simpl;intros. right. simpl in H. apply H.
...
Proof.
  intros n. induction n as [|n' IHn'].
    ...
    - rewrite evenb_s. destruct (evenb n') eqn:Hevenb. 
...
Proof.
  ...
  unfold not.
  split.
    - intros H1 H2. rewrite H2 in H1. rewrite <- beq_nat_refl in H1. inversion H1.
    - intros. destruct (beq_nat x y) eqn:Hbeq.
      + apply beq_nat_true_iff in Hbeq. apply H in Hbeq. destruct Hbeq.
      + reflexivity.
Qed.
(** [] *)
...
Proof.
  split.
    * induction l.
      - simpl. reflexivity.
      - simpl. rewrite -> andb_true_iff. intros. destruct H as [H0 H1]. 
       + split; simpl. apply H0. apply IHl in H1. apply H1.
   * induction l.
    - simpl. reflexivity.
    - simpl. rewrite -> andb_true_iff. intros. destruct H as [H0 H1].
      + split; simpl. apply H0. apply IHl in H1. apply H1. 
...
(* beq_nat is not used explicitily in the above proof *)
(** [] *)
