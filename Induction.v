...
They are both used to inductively define variables after an intro or intros. Variables can be
inductively defined using either in conjuction wtih as (or without). However, destruct will not provide 
an inductively defined hypothesis but induction will.
...
Proof.
  ...
  rewrite -> IHn.
  simpl.
    assert (H : forall m :nat, S (m + S (m + 0)) = S (S (m + (m + 0)))).
    intros m.
      rewrite <- plus_n_O.
      rewrite -> plus_comm.
        induction m.
