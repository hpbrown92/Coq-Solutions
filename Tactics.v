...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
Rewrite only needs exact match on the left. While apply 
...
(** Give a careful informal proof of [beq_nat_true], being as explicit
  ...
  _Theorem: beq_nat_true : forall n m, beq_nat n m = true -> n = m.
  
  _Proof: Like the previous example we prove this theorem induction on n by and later
          introducing m in a nonspecific form.
 
  _First: We start by matching the natural number n with 0. Then we prove our base case, 0 = m,
          by matching 0 with true and anything else with false. m is destructed to split it 
          into two subgoals. 

  _Next: We introduce m in a generic form along with the induction hypothesis n'. Here we are
         proving our recursive case by matching m with (n+1). Our variable m is matched in this 
         case with false when it m =0. Here beq_nat describes the relationship with 
         successor (S) m'. being matched with true. 
 
 _Finally: Our induction hypothesis (IHn') is applied to prove our generic case, 
          forall m : nat, beq_nat n' m = true -> n' = m. When n has value begin matching m with
          it's successor for all natural numbers: m. 
...
Proof.
  ...
  -intros. simpl. simpl in H. destruct n as [|n'].
    +inversion H.
...
(** **** Exercise: , optional (double_induction)  *)
...
Proof.
  intros f b.
  - destruct (b) eqn:Heqe1.
    destruct (f b) eqn:Heqe2.
